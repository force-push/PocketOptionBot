...
FIELDS: list[_F] = [
    ...
    _F("VARIABLE_STAKE_ENABLED", "variable_stake_enabled", "Safety & Trade Mode", "bool", False, False,
       "Variable Stake", "toggle",
       hint="Raise base stake only when the current pair/direction/expiry has proven positive edge"),
    _F("VARIABLE_STAKE_MIN_SAMPLES", "variable_stake_min_samples", "Safety & Trade Mode", "int", False, False,
       "Variable Stake Min Samples", "number",
       hint="Resolved trades needed in the current pair/direction/expiry bucket before stake can scale",
       step=1, min=1, max=500),
    _F("VARIABLE_STAKE_MIN_MULTIPLIER", "variable_stake_min_multiplier", "Safety & Trade Mode", "float", False, False,
       "Variable Stake Min Multiplier", "number",
       hint="Stake multiplier for proven-but-marginal positive-EV buckets before the trade is skipped",
       step=0.1, min=0.1, max=1.0),
    _F("VARIABLE_STAKE_MIN_EDGE", "variable_stake_min_edge", "Safety & Trade Mode", "float", False, False,
       "Variable Stake Min Edge", "number",
       hint="Probability margin above payout break-even before stake starts scaling",
       step=0.01, min=0.0, max=0.25),
    _F("VARIABLE_STAKE_FULL_EDGE", "variable_stake_full_edge", "Safety & Trade Mode", "float", False, False,
       "Variable Stake Full Edge", "number",
       hint="Probability margin above break-even that reaches the max multiplier",
       step=0.01, min=0.01, max=0.50),
    _F("VARIABLE_STAKE_MAX_MULTIPLIER", "variable_stake_max_multiplier", "Safety & Trade Mode", "float", False, False,
       "Variable Stake Max Multiplier", "number",
       hint="Cap for win-rate-based stake scaling before Martingale applies",
