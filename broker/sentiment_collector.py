...
class SentimentCollector:
    ...
    async def subscribe_pair(self, api_client: Any, pair: str, period: int = 5) -> None:
        """Explicitly send changeSymbol so the server starts pushing sentiment for *pair*.

        The library's get_candles() usually triggers this internally, but calling
        it here guarantees the subscription fires before the candle fetch.
        """
        try:
    ...
    def _process(self, raw: str) -> None:
        """Parse a raw WS message and update cache if it's a sentiment push."""
