...
class FlipParams:
    ...
    # Flip-specific distance bounds (separate from continuation — they have opposite optimal zones:
    # trend best=dist 1-2 ATR; flip best=dist 3+ ATR exhaustion reversals. 0/999=off).
    flip_atr_min: float = 0.0       # flip: require dist ≥ this×ATR (filters under-developed reversals)
    flip_atr_max: float = 999.0     # flip: skip if dist > this×ATR (usually left open for flips)
...
def evaluate_flip(df: pd.DataFrame, params: FlipParams = FlipParams()) -> FlipDecision:
    ...
    if flipped:
        ...
        if params.flip_atr_max < 999 and dist > params.flip_atr_max:
            return FlipDecision(None, None,
